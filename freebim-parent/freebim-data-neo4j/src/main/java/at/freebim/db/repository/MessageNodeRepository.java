...
/**
 * The repository for the node/class {@link MessageNode}. It extends
...
public interface MessageNodeRepository extends Neo4jRepository<MessageNode, Long> {
	...
	/**
	 * Gets a {@link List} of {@link MessageNode} from a {@link FreebimUser} that
	 * has the provided id and are greater than the time stamp.
	 *
	 * @param userId    the id of the user
	 * @param timestamp the time stamp
	 * @return {@link List} of {@link MessageNode}
	 */
	@Query("MATCH (msg:MessageNode), (u:FreebimUser)" + " WHERE ID(u)={0}"
			+ " AND ANY (r IN msg.roles WHERE r IN u.roles)" + " AND NOT (msg)-[:MESSAGE_CLOSED]->(u)"
			...
			+ " RETURN msg")
	List<MessageNode> getMessagesFor(Long userId, Long timestamp);
	 ...
	 */
	@Query("MATCH (msg:MessageNode)" + " WHERE \"ROLE_ANONYMOUS\" IN msg.roles"
			...
			+ " RETURN msg")
	List<MessageNode> getMessagesForAnonymous(Long timestamp);
}
