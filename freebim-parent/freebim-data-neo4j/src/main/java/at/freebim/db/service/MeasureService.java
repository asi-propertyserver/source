...
public interface MeasureService extends ContributedBaseNodeService<Measure>, BsddObjectService<Measure> {
	 ...
	 * @param valueList
	 * @param prefix
	 * @param existingMeasures List of Measure objects to find potential existing measures. 
	 * @param library Library an eventually new created Measure should reference.
	 * @return the created {@link Measure} object/node
	 */
	@Secured({"ROLE_EDIT", "ROLE_ADMIN"})
	public Measure getMeasureFor(final DataType dataType, final Unit unit, final ValueList valueList, 
			String prefix, 
	 ...
	 * @return a {@link List} of all the {@link Measure}s that have the provided name
	 */
	@Secured({"ROLE_EDIT", "ROLE_ADMIN"})
