...
/**
 * The service for bsdd things.
 * It extends {@link BsddService}.
 * 
...
public class BsddServiceImpl implements BsddService {
	...
	/* (non-Javadoc)
	 * @see at.freebim.db.service.BsddService#didBsddGuidChange(at.freebim.db.domain.base.BsddObject)
	 */
	@Override
	@Transactional(readOnly = true)
	public boolean didBsddGuidChange(BsddObject obj) {
		if (obj != null) {
			Node node = this.relationService.getTemplate().getNode(obj.getNodeId());
			BaseNode bn = this.relationService.createTreeNode(node);
			if (bn != null && BsddObject.class.isAssignableFrom(bn.getClass())) {
				BsddObject savedObj = (BsddObject) bn;
