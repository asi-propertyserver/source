...
/**
 * The service for the relation/class {@link BaseRel}. This service extends
 * {@link BaseService}.
 *
...
public interface RelationService extends BaseService<BaseRel<?, ?>> {
	...
	void deleteAllRelationsFor(BaseNode node, String reltype);
	 ...
	 * @param onlyValid If set to <code>true</code> all nodes on path have to be
	 *                  valid.
	 * @param max       Maximum number of returned paths.
	 * @return {@link List} of all found shortest paths.
	 */
	List<SimplePathResult> getAllPaths(Long nodeId, boolean onlyValid, Long max);

	/**
	 * Get all ingoing or outgoing relations and nodes that are related to the node
	 * that has the provided id.
	 *
	 * @param nodeId the id of the node
	 * @return a {@link ArrayList} of related relations and node
	 */
	ArrayList<RelationResult> getAllRelatedInOut(Long nodeId);
	 ...
	 */
	boolean isMaterial(Long nodeId);

	/**
	 * Update the relations from a node that has the provided id.
	 *
	 * @param nodeId         the id of the node from which the relations should be
	 *                       updated
	 ...
	 */
	@Secured({ "ROLE_EDIT", "ROLE_ADMIN" })
	<FROM extends NodeIdentifyable, TO extends NodeIdentifyable> UpdateRelationsResult<FROM, TO> updateRelations(
			Long nodeId, Relations[] relationsArray, Class<? extends BaseNode> clazz);

	/**
	 * Create a node for the tree or {@link BaseNode} out of the class {@link Node}.
	 *
	 * @param n the node
	 ...
	 */
	UuidIdentifyable findByFreebimId(String freebimId);
	 ...
	 */
	ArrayList<BaseNode> findByBsddGuid(String bsddGuid);
	 ...
	 */
	ArrayList<StatedBaseNode> findByState(String searchstring);

	/**
	 * Fully loades the single entity or collection thereof which has been loaded
	 * lazily (i.e. just with the id but w/o values).
	 *
	 * @param child the node that will be fully loaded
	 * @return the fully loaded {@link BaseNode}
	 */
	BaseNode fetch(BaseNode child, Class<? extends BaseNode> clazz);
	 ...
	 */
	UuidIdentifyable getByUuid(String uuid);

	/**
	 * Get a node by its id.
	 *
	 * @param nodeId the id of the node
	 * @param clazz the class of the node that should be retrieved
	 * @return the {@link BaseNode} that has the provided id
	 */
	BaseNode getNodeById(Long nodeId, Class<? extends BaseNode> clazz);
	 ...
	 * @return the fully loaded {@link BaseNode}
	 */
	<R extends BaseRel<? extends NodeIdentifyable, ? extends NodeIdentifyable>> BaseNode getRelatedNode(BaseNode src,
			R rel, String dir);
	 ...
	 */
	@Secured({ "ROLE_EDIT", "ROLE_ADMIN" })
	HasParameter saveDefaultForParameter(HasParameter rel);
	 ...
	 */
	@Secured({ "ROLE_EDIT", "ROLE_ADMIN" })
	<FROM extends NodeIdentifyable, TO extends NodeIdentifyable> void createIfNotExists(BaseRel<FROM, TO> rel,
	 ...
	 * @author rainer.breuss@uibk.ac.at
	 */
	class UpdateRelationsResult<FROM extends NodeIdentifyable, TO extends NodeIdentifyable> {
		/**
