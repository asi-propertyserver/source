...
public class AsyncNgramCreatorImpl implements Runnable, AsyncNgramCreator {
	...
	/**
	 * This method should be called before the object is destroyed to gently stop the threads
	 * and save nodes that are still in the queue.
	 */
	@PreDestroy
	public void stop() {
			...
			do {
				...
				if (nodeId != null) {
					...
					try {
						...
						// do it again, sam!
						logger.info("Error creating Ngram's for node, nodeId = {}, try it again after a second ...", nodeId);
						try {
							Thread.sleep(1000L);
							this.add(nodeId);
	...
	/* (non-Javadoc)
	 * @see at.freebim.db.service.AsyncNgramCreator#add(at.freebim.db.domain.base.BaseNode)
	 */
	@Override
	...
	/**
	 * Polls a element from the queue. If there is no element in the queue
	 * and <code>running</code> is true then the {@link ConcurrentLinkedQueue} will wait.
	 * The method returns <code>null</code> if no element is in the list 
	 * or <code>running</code> is false.
	 * 
	 * @return the head of the queue. if <code>null</code> is returned the 
	 * queue is empty or <code>running</code> is false.
	 * 
