...
/**
 * This service handles a user of the system. A user is represented by the
 * node/class {@link FreebimUser}. This service extends
 * {@link LifetimeBaseNodeServiceImpl} and implements
 * {@link FreebimUserService}.
 *
...
public class FreebimUserServiceImpl extends LifetimeBaseNodeServiceImpl<FreebimUser> implements FreebimUserService {
	...
	@Override
	@Autowired
	public void setRepository(Neo4jRepository<FreebimUser, Long> r) {
		this.repository = r;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see at.freebim.db.service.FreebimUserService#register(java.lang.String,
	 * java.lang.String, at.freebim.db.domain.base.Role[],
	 * at.freebim.db.domain.Contributor)
	 */
	@Override
	@Transactional
	public FreebimUser register(String login, String password, Role[] roles, Contributor contributor)
	...
	public void init() {
		...
		if (user == null) {
			...
			Authentication auth = new UsernamePasswordAuthenticationToken(user.getUsername(), this.password,
					user.getRoles());
			SecurityContextHolder.getContext().setAuthentication(auth);

			try {
				Contributor freeBimContributor = this.contributorService
		...
		if (guest == null) {
			...
			guest.setPassword(this.passwordEncoder.encode(this.guestPassword));
	 ...
	 * (non-Javadoc)
	 * 
	 * @see at.freebim.db.service.FreebimUserService#test(java.lang.String,
	 * java.lang.String, at.freebim.db.domain.base.Role[])
	 */
	@Override
	@Transactional(readOnly = true)
	...
	/**
	 * This methode is overriden to enforece javax.validation
	 */
	@Override
	public FreebimUser save(FreebimUser node) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see at.freebim.db.service.impl.LifetimeBaseNodeServiceImpl#filterRequest(at.
	...
	/**
	 * Check the password and hash it if necessary. Additionally connect the
	 * {@link Contributor}.
	 *
	...
	private void manageBeforeSave(FreebimUser node) {
		if (node != null) {
			if (node.getPassword().length() < 32) { // length of hashed password should be 60 put user input will be
													// limited to 31 characters
				node.setPassword(this.passwordEncoder.encode(node.getPassword()));
			}
	...
	private void manageRoleContributor(FreebimUser node) {
		...
		if (node != null) {
			...
			if (node.getContributor() != null
					&& (node.getContributor().getValidFrom() != null && node.getContributor().getValidFrom() <= now)
					&& (node.getContributor().getValidTo() == null || node.getContributor().getValidTo() > now)) {
	...
	public ArrayList<FreebimUser> getAllRelevant() {
		...
		ArrayList<FreebimUser> res = new ArrayList<>();

		StringBuilder b = new StringBuilder();

		b.append("MATCH (n:FreebimUser)");
		b.append(" WHERE n.validFrom <= {now} AND ((n.validTo IS NULL) OR n.validTo > {now})");
		...
		while (iter.hasNext()) {
			...
			FreebimUser n = (FreebimUser) map.get("n");
			res.add(n);
