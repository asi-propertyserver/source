 ...
 * {@link LibraryService}, {@link ApplicationEventPublisherAware} and
 ...
 */
@Service
public class LibraryServiceImpl extends HierarchicalBaseNodeServiceImpl<Library>
		implements LibraryService, ApplicationEventPublisherAware, ApplicationListener<LibraryUpdateEvent> {
	...
	@Override
	@Autowired
	public void setRepository(Neo4jRepository<Library, Long> r) {
		this.repository = r;
	}
	 ...
	 * 
	 * @see
	 * at.freebim.db.service.LibraryService#getForContributor(at.freebim.db.domain.
	 * Contributor)
	 */
	@Override
	@Transactional(readOnly = true)
	 ...
	 *
	 * @param library the {@link Library} for which the ifc structure will be
	 *                created.
	 */
	...
	private void createIfcStructure(Library library) {
		...
		HierarchicalBaseNode ifcMaterialConstSet = createNodeAsChildOf(ifcMaterialDef, "IfcMaterialConstituentSet",
				library);
		HierarchicalBaseNode ifcMaterialLayer = createNodeAsChildOf(ifcMaterialDef, "IfcMaterialLayer", library);
		HierarchicalBaseNode ifcMaterialLayerWO = createNodeAsChildOf(ifcMaterialLayer, "IfcMaterialLayerWithOffsets",
				library);
		...
		HierarchicalBaseNode ifcMaterialProfileWO = createNodeAsChildOf(ifcMaterialProfile,
				"IfcMaterialProfileWithOffsets", library);
		HierarchicalBaseNode ifcMaterialProfileSet = createNodeAsChildOf(ifcMaterialDef, "IfcMaterialProfileSet",
				library);

	}

	/**
	 * The provided {@link HierarchicalBaseNode} is the {@link ParentOf} the
	 * {@link Component} that will be created from the provided name. That
	 * {@link Component} {@link References} the provided {@link Library}.
	 *
	 ...
	 * (non-Javadoc)
	 * 
	 * @see at.freebim.db.service.LibraryService#getReferencedNodes(java.lang.Long,
	 * java.lang.String)
	 */
	@Override
	@Transactional(readOnly = true)
	public List<LibraryReference> getReferencedNodes(Long libraryNodeId, String refIdName) {
		List<LibraryReference> res = new ArrayList<>();

		String query = "MATCH (lib) WHERE ID(lib)={libraryNodeId} MATCH (n)-[ref:" + RelationType.REFERENCES
		...
		while (iter.hasNext()) {
			...
			if (n != null) {
				BaseNode bn = n;// this.relationService.createTreeNode(n);
	 ...
	 * 
	 * @see
	 * at.freebim.db.service.LibraryService#getReferencedNodesForRefId(java.lang.
	 * Long, java.lang.String, java.lang.String)
	 */
	@Override
	@Transactional(readOnly = true)
	public List<UuidIdentifyable> getReferencedNodesForRefId(Long libraryNodeId, String refIdName, String refId) {
		List<UuidIdentifyable> res = new ArrayList<>();

		StringBuilder b = new StringBuilder();
		b.append("MATCH (lib) WHERE  ID(lib)={libraryNodeId} MATCH (n)-[ref:");
	 ...
	 * 
	 * @see
	 * at.freebim.db.service.impl.ContributedBaseNodeServiceImpl#filterAfterInsert(
	...
	/**
	 * Get the smallest and the greatest id of the nodes that {@link References} the
	 * provided {@link Library}.
	 *
	 * @param libraryNodeId
	 * @return first element of the array is the min value the second is the max
	 *         value
	 */
	@Transactional(readOnly = true)
	private long[] getMinMaxId(long libraryNodeId) {
		...
		params.put("libraryNodeId", libraryNodeId);
		StringBuilder b = new StringBuilder();
		b.append("MATCH (lib) WHERE  ID(lib)={libraryNodeId} MATCH (lib)<-[:");
		b.append(RelationType.REFERENCES);
	...
	/**
	 * Delete all relations ref to nodes that {@link References} the
	 * {@link Library}. (lib)<-[r:References]-(x)-(ref)-()
	 *
	...
	private long clearRelations(long min, long delta, long high, Map<String, Object> params) {
		...
		Iterable<Map<String, Object>> result = this.relationService.getTemplate().query(query, params);
		if (result != null) {
			Iterator<Map<String, Object>> iter = result.iterator();
			while (iter.hasNext()) {
				Map<String, Object> map = iter.next();
	...
	private long clearNodes(long min, long delta, long high, Map<String, Object> params) {
		...
		max = ((max > high) ? high : max);
		b.setLength(0);
		b.append("MATCH (lib) WHERE ID(lib)={libraryNodeId} MATCH (lib)<-[r:");
		b.append(RelationType.REFERENCES);
		b.append("]-(x) WHERE ID(x) >= ");
		b.append(min);
		b.append(" AND ID(x) < ");
			...
			if (user != null && user.getContributor() != null) {
				...
				if (c != null) {
					if (this.contributorService.test(c,
						...
						logger.info("Contributor [{}] did trigger update Library nodeId={} event.", c.getName(),
								libraryNodeId);
						done = true;
					}
				}
